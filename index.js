...
const INSTALL_DATE_FIELD=process.env.INSTALL_DATE_FIELD||'install_date';
...
function getInstallDate(opp){
  const fields=opp.customFields||opp.custom_fields||[];
  for(const f of fields){
    const key=(f.key||f.fieldKey||'').toLowerCase();
    const id=(f.id||'').toLowerCase();
    if(key.includes('install_date')||key.includes('install date')||id.includes('install_date')){
      return f.value||f.fieldValue||null;
    }
  }
  // Also check top level
  if(opp.install_date)return opp.install_date;
  return null;
}
...
app.get('/api/calendar',async(req,res)=>{
  try{
    ...
    for(const opp of opps){
      const pipeline=(opp.pipeline?.name||opp.pipelineName||opp.pipeline_name||'').toLowerCase();
      if(!pipeline.includes('knocking')&&!pipeline.includes('estimator'))continue;
      const rawDate=getInstallDate(opp);
      const date=parseDate(rawDate);
      if(!date)continue;
      const revenue=parseFloat(opp.monetaryValue||opp.value||opp.opportunity_value||0)||0;
      ...
      byDate[date].jobs.push({
        ...
        revenue,
        pipeline:opp.pipeline?.name||opp.pipelineName||'',
        stage:opp.stage?.name||opp.pipelineStage?.name||''
      });
    }
...
.cell{min-height:100px;padding:8px;border-right:1px solid #161616;border-bottom:1px solid #161616;cursor:pointer;transition:background .15s;position:relative}
...
async function loadData(){
  try{
    ...
    document.getElementById('calGrid').innerHTML='<div class="error-msg" style="grid-column:span 7">Error loading data: '+e.message+'<br><button onclick="loadData()" style="margin-top:12px;background:#1a1a1a;border:1px solid #333;color:#ccc;padding:8px 16px;border-radius:6px;cursor:pointer">Retry</button></div>';
  }
}
...
function renderCalendar(){
  ...
  for(var d=1;d<=last.getDate();d++){
    ...
    html+='<div class="'+cls+'"'+(hasJobs?' onclick="showDay(''+dateStr+')"':'')+' style="'+(hasJobs?'cursor:pointer':'')+'">';
